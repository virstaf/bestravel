...
test('getFormattedDate - formats correctly', () => {
  const date = '2025-05-20';
  const formatted = getFormattedDate(date);
  assert.strictEqual(formatted, 'May 20, 2025');
});

test('getFormattedDateTime - formats correctly', () => {
  const date = '2025-05-20T10:30:00';
  const formatted = getFormattedDateTime(date);
  assert.ok(formatted.includes('May 20, 2025'));
