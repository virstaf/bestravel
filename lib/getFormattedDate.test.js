...
test('getFormattedDate', async (t) => {
  await t.test('formats valid date correctly', () => {
    const result = getFormattedDate('2023-05-15T10:30:00Z');
    // We expect "May 15, 2023" based on en-US locale
...
test('getFormattedDateTime', async (t) => {
  await t.test('formats valid datetime correctly', () => {
    const result = getFormattedDateTime('2023-05-15T10:30:00Z');
    // "May 15, 2023, 10:30 AM" (exact format can vary slightly by env but should be consistent)
    // Using includes to be safe about slight variations in space/punctuation
