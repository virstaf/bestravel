...
test("getFormattedDateTime", async (t) => {
  await t.test("formats a valid date-time string", () => {
    ...
    // Using includes because exact output can vary by environment locale settings (e.g. AM/PM vs 24h)
...
test("getShortDate", async (t) => {
  await t.test("formats a valid date string", () => {
    const date = "2023-10-27";
    assert.strictEqual(getShortDate(date), "Oct 27, 2023");
  });

  await t.test("returns N/A for invalid input", () => {
    assert.strictEqual(getShortDate("not-a-date"), "N/A");
  });
});
