...
test("getFormattedDate", async (t) => {
  await t.test("formats valid date correctly", () => {
    ...
    // We check for the components of the date to avoid timezone issues
    // although getFormattedDate (without time) is usually more stable across timezones
...
test("getFormattedDateTime", async (t) => {
  await t.test("formats valid datetime correctly", () => {
    const result = getFormattedDateTime("2024-05-16T12:30:00Z");
    // Check for specific date components rather than an exact string
    // to make the test more robust against timezone differences.
    assert.ok(result.includes("May 16") || result.includes("May 15"));
    assert.ok(result.includes("2024"));
    // Verify that it contains something that looks like time
