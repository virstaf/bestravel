...
test("getFormattedDate", async (t) => {
  await t.test("formats a valid date string", () => {
    const result = getFormattedDate("2023-05-15T10:00:00Z");
...
test("getFormattedDateTime", async (t) => {
  await t.test("formats a valid date string with time", () => {
    const result = getFormattedDateTime("2023-05-15T10:30:00Z");
    // The exact format might depend on the environment's locale, but it should contain these elements
    assert.match(result, /May 15, 2023/);
    assert.match(result, /10:30/);
  });
