...
test('getFormattedDate', async (t) => {
  await t.test('formats a valid date string', () => {
    const result = getFormattedDate('2024-01-01');
...
test('getFormattedDateTime', async (t) => {
  await t.test('formats a valid date time string', () => {
    // Note: The exact output format can depend on the environment's locale settings for time (AM/PM vs 24h)
    // but the year/month/day should be consistent.
    const result = getFormattedDateTime('2024-01-01T12:00:00');
    assert.ok(result.includes('Jan 1, 2024'));
