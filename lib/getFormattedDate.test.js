...
test("getFormattedDate - validation", async (t) => {
  await t.test("returns correct format for valid date", () => {
    const date = "2024-05-16T12:00:00Z";
    const formatted = getFormattedDate(date);
    // Use includes for timezone robustness (May 16 or May 15 depending on local TZ)
    assert.ok(
...
test("getFormattedDateTime - validation", async (t) => {
  await t.test("returns correct format for valid date", () => {
    const date = "2024-05-16T12:00:00Z";
    ...
    // Check for time component (contains ":" and "AM" or "PM")
