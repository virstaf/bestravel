...
test("getFormattedDate", async (t) => {
  await t.test("formats a valid date string", () => {
    ...
    // Use includes to be resilient to environment-specific space characters or variations
...
test("getFormattedDateTime", async (t) => {
  await t.test("formats a valid date time string", () => {
    ...
    // Check for date part, time part varies by timezone but should be present
    assert.ok(result.includes("May 16, 2024"));
  });
