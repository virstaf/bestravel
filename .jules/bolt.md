...
## 2026-05-15 - [Eliminating Server-Side Waterfalls in Dashboard]

**Learning:** In Next.js App Router, multiple Server Components within a page that independently fetch data create a rendering waterfall. Hoisting these fetches to the parent page and executing them in parallel with `Promise.all` ensures that the page wait time is limited by the slowest request rather than the sum of all requests, significantly improving Time to First Byte (TTFB).
**Action:** Hoist independent data fetches from nested Server Components to the parent page and parallelize them using `Promise.all`.
